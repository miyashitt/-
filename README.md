...
<body>
  ...
  <script>
    (() => {
      ...
      const customAlertOverlay = document.getElementById("custom-alert-overlay"); // カスタムアラートのオーバーレイ
      const customAlertOkBtn = document.getElementById("custom-alert-ok-btn"); // カスタムアラートのOKボタン
      ...
      function showCustomAlert(message) {
        ...
        customAlertOverlay.style.display = "flex"; // Flexboxで中央配置
      }
      ...
      function startVirus() {
        ...
        const runVirusSimulation = () => {
          if (logIndex < totalLogSteps) {
            ...
            // alert() の代わりにカスタムアラートを表示
            showCustomAlert("すべてのデータを削除しました。");
            localStorage.setItem(localStorageKey, "1");
            // リリーフ画面への遷移はカスタムアラートのOKボタンが押された後に行う
          }
        };
      ...
      // カスタムアラートのOKボタンクリックイベント
      customAlertOkBtn.addEventListener("click", () => {
        hideCustomAlert(); // アラートを非表示にする
        showReliefScreen(); // その後、リリーフ画面に遷移
      });
